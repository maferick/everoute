...
<div class="container">
    ...
    <section class="card">
        <form method="post">
            ...
            <div class="toggles">
                ...
                <label><input type="checkbox" name="debug" <?= !empty($_POST['debug']) ? 'checked' : '' ?>> Enable debug output</label>
            </div>
            <label>
    ...
    <section class="card">
        ...
        <?php if ($result && empty($result['error'])): ?>
            ...
            <?php else: ?>
                <h3>Engine Output</h3>
                <?php if (!empty($result['best']) && $result['best'] !== 'none'): ?>
                    <p class="muted">Best option: <?= htmlspecialchars((string) $result['best'], ENT_QUOTES) ?></p>
                <?php endif; ?>
                <?php foreach ($engineRoutes as $label => $route): ?>
                    <?php if ($route === null): ?>
                        <?php continue; ?>
                    <?php endif; ?>
                    <h4><?= htmlspecialchars($label, ENT_QUOTES) ?> Route</h4>
                    <?php if (!empty($route['feasible'])): ?>
                        <div class="stats">
                            <div>Total gates: <?= htmlspecialchars((string) ($route['total_gates'] ?? '0'), ENT_QUOTES) ?></div>
                            <div>Total jump LY: <?= htmlspecialchars((string) ($route['total_jump_ly'] ?? '0'), ENT_QUOTES) ?></div>
                            <div>Total cost: <?= htmlspecialchars((string) ($route['total_cost'] ?? '0'), ENT_QUOTES) ?></div>
                            <div>Nodes explored: <?= htmlspecialchars((string) ($route['nodes_explored'] ?? '0'), ENT_QUOTES) ?></div>
                            <div>Illegal systems filtered: <?= htmlspecialchars((string) ($route['illegal_systems_filtered'] ?? '0'), ENT_QUOTES) ?></div>
                        </div>
                        <?php if (!empty($route['fatigue'])): ?>
            ...
            <?php if (!empty($result['debug'])): ?>
                <details>
                    ...
                    <pre><?= htmlspecialchars(json_encode($result['debug'], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES), ENT_QUOTES) ?></pre>
                </details>
            <?php endif; ?>
