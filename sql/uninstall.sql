DROP TABLE IF EXISTS risk_import_jobs;
DROP TABLE IF EXISTS risk_meta;
DROP TABLE IF EXISTS route_cache;
DROP TABLE IF EXISTS precompute_checkpoints;
DROP TABLE IF EXISTS jump_midpoint_candidates;
DROP TABLE IF EXISTS jump_constellation_edges;
DROP TABLE IF EXISTS jump_constellation_portals;
DROP TABLE IF EXISTS jump_neighbors;
DROP TABLE IF EXISTS gate_distances;
DROP TABLE IF EXISTS constellation_hierarchy;
DROP TABLE IF EXISTS region_hierarchy;
DROP TABLE IF EXISTS static_meta;
DROP TABLE IF EXISTS chokepoints;
DROP TABLE IF EXISTS kill_events;
DROP TABLE IF EXISTS system_risk;
DROP TABLE IF EXISTS sde_meta;
DROP TABLE IF EXISTS stations;
DROP TABLE IF EXISTS stargates;
DROP TABLE IF EXISTS systems;
